...
class LLaDAModelLM(PreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        """
        # If past_key_values is used, only use the last token of input_ids
        if past_key_values is not None:
            input_ids = input_ids[:, -1:]
        
        # If inputs_embeds are provided, only use those
    ...
    def block_distillation_loss(self, input_ids, attention_mask, max_gen_length=1024, block_length=64, threshold_high=0.9, threshold_low=0.6, eos_token_id=126081, reference_model=None, beta=0.1):
        ...
        # Prefill phase to get initial past_key_values (shared prefix)
        prefill_outputs = self.prefill_phase(input_ids, block_length)
        past_key_values = prefill_outputs['past_key_values']
        logits = prefill_outputs['logits']
        
        # 立即释放不再需要的prefill_outputs
        del prefill_outputs
        
        block_x = torch.full((batchsize, block_length), self.mask_id, dtype=torch.long, device=self.device)
        
        # 只保存当前block的ids，不保存完整序列
        teacher_block_ids = block_x.clone()
        teacher_block_ids = self.unmask_function_greedy(logits=logits, x=teacher_block_ids, threshold=threshold_high)
        
        student_block_ids = block_x.clone()
        student_block_ids = self.unmask_function_greedy(logits=logits, x=student_block_ids, threshold=threshold_low)
        
        # 释放logits
        del logits
        
        # Initialize reference model branch if provided
        ref_past_key_values = None
        reference_block_ids = None
        if reference_model is not None:
            with torch.no_grad():
                ref_prefill_outputs = reference_model.prefill_phase(input_ids, block_length)
                ref_past_key_values = ref_prefill_outputs['past_key_values']
                ref_logits = ref_prefill_outputs['logits']
                reference_block_ids = block_x.clone()
                reference_block_ids = reference_model.unmask_function_greedy(logits=ref_logits, x=reference_block_ids, threshold=threshold_high)
                del ref_logits, ref_prefill_outputs
        
        for i in range(max_num_blocks):
            # Teacher generation with high confidence threshold
            with torch.no_grad():
                # Iteratively fill the block using the high-confidence policy
                teacher_outputs = None
                while (teacher_block_ids == self.mask_id).sum() > 0:
                    teacher_outputs = self(
                        input_ids=teacher_block_ids,
                        past_key_values=past_key_values,
                        use_cache=True,
                        return_dict=True
                    )
                    teacher_block_ids = self.unmask_function_greedy(logits=teacher_outputs.logits, x=teacher_block_ids, threshold=threshold_high)
                
                # If teacher_outputs is None (block was already filled), run one forward pass to get logits
                ...
                # The final logits from the teacher for the generated block - detach immediately
                teacher_logits = teacher_outputs.logits.detach()
                
                # Reference model generation with high confidence threshold (0.9)
                reference_logits = None
                if reference_model is not None:
                    ref_outputs = None
                    while (reference_block_ids == self.mask_id).sum() > 0:
                        ref_outputs = reference_model(
                            input_ids=reference_block_ids,
                            past_key_values=ref_past_key_values,
                            use_cache=True,
                            return_dict=True
                        )
                        reference_block_ids = reference_model.unmask_function_greedy(logits=ref_outputs.logits, x=reference_block_ids, threshold=threshold_high)
                    
                    # If ref_outputs is None (block was already filled), run one forward pass to get logits
                    ...
                    # The final logits from the reference model - detach immediately
                    reference_logits = ref_outputs.logits.detach()
                
            # Student generation with low confidence threshold
            student_outputs = None
            while (student_block_ids == self.mask_id).sum() > 0:
                student_outputs = self(
                    input_ids=student_block_ids,
                    past_key_values=past_key_values,
                    use_cache=True,
                    return_dict=True
                )
                student_block_ids = self.unmask_function_greedy(logits=student_outputs.logits, x=student_block_ids, threshold=threshold_low)
            
            # If student_outputs is None (block was already filled), run one forward pass to get logits
            ...
            # Use explicit KL divergence formulas with in-place operations
            # KL(student || teacher) = Σ_s student_probs * (log student_probs - log teacher_probs)
            with torch.amp.autocast(device_type=self.device.type, enabled=False):
                log_student = F.log_softmax(student_logits, dim=-1)
                log_teacher = F.log_softmax(teacher_logits, dim=-1)
                
                # We only care about the loss on the tokens that were actually generated
                mask = (teacher_block_ids != self.mask_id)
                
                # Calculate KL(student || teacher) more efficiently
                # Only compute on non-masked positions
                if mask.any():
                    kl_student = F.kl_div(log_student[mask], log_teacher[mask].exp(), reduction='batchmean', log_target=False)
                    total_loss += kl_student.detach()
                
                # Calculate KL(teacher || reference) for reference regularization
                if reference_model is not None and reference_logits is not None:
                    log_ref = F.log_softmax(reference_logits, dim=-1)
                    
                    # Use the same mask for reference model loss
                    if mask.any():
                        kl_ref = F.kl_div(log_teacher[mask], log_ref[mask].exp(), reduction='batchmean', log_target=False)
                        total_ref_loss += kl_ref.item()
                
                # 立即释放大的张量
                del log_student, log_teacher, student_logits, teacher_logits
                if reference_logits is not None:
                    del log_ref, reference_logits
            
            num_loss_steps += 1

            # Check for EOS token
            if (teacher_block_ids == eos_token_id).any():
                break
            
            # Reset block_ids for next iteration
            teacher_block_ids = block_x.clone()
            student_block_ids = block_x.clone()
        ...
        # Combine the losses with beta coefficient
        avg_student_loss = total_loss / num_loss_steps if num_loss_steps > 0 else torch.tensor(0.0).to(self.device)
        
        if reference_model is not None and num_loss_steps > 0:
            avg_ref_loss = total_ref_loss / num_loss_steps
            combined_loss = avg_student_loss + beta * avg_ref_loss
            # 返回一个字典，包含所有损失信息
            return {
                'loss': combined_loss,
            ...
            # 如果没有参考模型，返回字典格式保持一致
            return {
                'loss': avg_student_loss,
                'student_loss': avg_student_loss,
                'ref_loss': torch.tensor(0.0).to(self.device),
                'num_steps': num_loss_steps
            }
