...
def compute_loss(batch, base_model, config):
    ...
    for i in range(max_num_blocks):
        ...
        if mask_bool.any():
            ...
            mse_student = F.mse_loss(student_hidden_valid, teacher_hidden_valid, reduction='mean')
            total_loss = mse_student if total_loss is None else total_loss + mse_student
            del student_hidden_valid, teacher_hidden_valid, mask_expanded
        
        del student_hidden_states, teacher_hidden_states, student_outputs
        num_loss_steps += 1
...
def main():
    ...
    for epoch in range(config.total_epochs):
        ...
        for batch in train_pbar:
            ...
            if getattr(config, 'use_wandb', False):
                accelerator.log({
                    "train/loss": metrics['loss'], "train/mse_student": metrics['student_loss'],
                    "train/lr": metrics['lr'], "train/grad_norm": metrics['grad_norm'], 
                    "epoch": epoch + 1
                }, step=global_step)
            ...
            if accelerator.is_main_process:
                tqdm.write(f"[Train] step {global_step} | loss={metrics['loss']:.10f} "
                          f"mse_stu={metrics['student_loss']:.4f} "
                          f"lr={metrics['lr']:.10f}")
